...
const translations = {
  en: {
    ...
    svcWorkshopTitle: "Engineering practice workshop",
    svcWorkshopBody: "A half-day or full-day session, remote by default, on the practices behind the XingAI products — adapted to your stack and to the decision you are facing now.",
    svcWorkshopTopicsHeading: "Topics to choose from",
    svcWorkshopTopics: "Writing decisions down: architecture decision records that state the context, the tradeoffs and the limits|Evaluating LLM features: datasets, metrics and regression gates that can block a bad release|Taking MCP to production: gateways, permission manifests and approval gates|One answer, computed in one place: worker-writes / API-reads for AI products",
  ...
  zh: {
    ...
    svcWorkshopTitle: "工程实践工作坊",
    svcWorkshopBody: "半天或一天，默认远程，讲 XingAI 产品背后的工程做法，并按你们的技术栈和眼前要做的决定来调整内容。",
    svcWorkshopTopicsHeading: "可选主题",
    svcWorkshopTopics: "把决策写下来：写清背景、取舍和边界的架构决策记录（ADR）|评测大模型功能：数据集、指标，以及能拦住坏版本的回归门禁|把 MCP 推上生产：网关、权限清单和审批门禁|一个答案只在一处计算：AI 产品的“worker 写、API 读”架构",
  ...
  ko: {
    ...
    svcWorkshopTitle: "엔지니어링 실무 워크숍",
    svcWorkshopBody: "반나절 또는 하루, 기본은 원격으로 진행합니다. XingAI 제품을 만든 엔지니어링 방식을 여러분의 기술 스택과 지금 내려야 할 결정에 맞춰 다룹니다.",
    svcWorkshopTopicsHeading: "선택 가능한 주제",
    svcWorkshopTopics: "결정을 기록하기: 배경, 트레이드오프, 한계를 적은 아키텍처 결정 기록(ADR)|LLM 기능 평가: 데이터셋, 지표, 나쁜 릴리스를 막는 회귀 게이트|MCP를 프로덕션으로: 게이트웨이, 권한 목록, 승인 게이트|답은 한 곳에서만 계산: AI 제품의 worker 쓰기 / API 읽기 구조",
